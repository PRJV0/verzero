...
- ZERO EFFORT si dichiara SEMPRE con la sua definizione, mai da solo: "Zero effort non vuol dire zero coinvolgimento: vuol dire che ti chiediamo solo cio che solo tu puoi darci — i documenti che hai gia — e ci occupiamo di tutto il resto." Quantificare dove possibile ("un'ora del tuo tempo, il resto lo fa il Motore"). Vietate le formule che promettono che il cliente non faccia nulla.
...
**12.O-bis — SUPPORTO ALL'AUDIT DI CERTIFICAZIONE (nuovo servizio a catalogo).**
- COS'E: intervento mirato dopo la visita dell'organismo di certificazione. Il cliente carica i rilievi ricevuti (osservazioni, raccomandazioni, non conformita minori e maggiori) e Verzero adegua la documentazione del sistema di gestione perche risponda punto per punto. Funziona anche su documenti NON prodotti da Verzero.
- PREZZO: one shot 390 euro fascia micro, con i gradini standard della matrice dimensionale (piccola +20%, media +50%, grande su richiesta). NESSUN CANONE e nessun rinnovo: e il primo servizio del listino a forma "una tantum", e il funnel non mostra alcuna scelta di formula.
- OUTPUT: documentazione revisionata nei punti oggetto di rilievo + nota di risposta ai rilievi con il requisito di norma richiamato; verifica umana OBBLIGATORIA su tutto il pacchetto, con firma del professionista.
- VINCOLI (non negoziabili): non si promette mai l'esito dell'audit, che appartiene esclusivamente all'organismo; non si contatta l'ente per conto del cliente ne si interloquisce con gli auditor — l'indipendenza della parte terza resta intatta; si adegua la documentazione, l'attuazione delle azioni correttive in azienda resta al cliente.
- RICHIAMI: in fondo alle pagine dei servizi certificabili (ISO 9001, ISO 14001, UNI/PdR 125; a seguire UNI ISO 21401 quando esce dalla roadmap) compare la riga "Hai gia un audit in corso? Possiamo aiutarti anche se i documenti non li abbiamo fatti noi". Nell'area riservata, sui moduli attivi dei percorsi certificabili, compare l'azione contestuale "Hai ricevuto rilievi dall'ente? Caricali qui" verso l'acquisto del servizio; l'upload effettivo dei rilievi arriva in fase 2 con il resto della pipeline documentale.
- DATABASE: gli ordini una tantum richiedono `formula = 'una_tantum'` con `prezzo_canone` nullo e `prezzo_una_tantum` valorizzato (vincolo `prezzo_coerente`), migrazione 20260806120000.
