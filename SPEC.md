...
**12.E.1 — HUB DOCUMENTI: COME E STATO REALIZZATO (19/08/2026).**
- ARCHIVIO: bucket privato `documenti` su Supabase Storage, una cartella per organizzazione (il primo pezzo del percorso e l'id dell'org, ed e su quello che la RLS confronta). Tabella `documents` con tipo, tipo_confermato e stato; permessi a colonna: il cliente corregge il tipo e archivia, non riscrive percorso o dimensione. Test RLS estesi a 42: A non scrive nella cartella di B, B non scarica il file di A nemmeno conoscendone il percorso, il consulente con mandato revocato non vede nulla.
- CARICAMENTO: drag&drop e multiplo, il file va DIRETTAMENTE nel bucket dal browser (non passa dal nostro server). PDF e immagini fino a 20 MB, validazioni con messaggi che dicono cosa fare; un file che fallisce non ferma gli altri; se la registrazione fallisce il file viene rimosso, mai orfani nel bucket.
- RICONOSCIMENTO (solo dal nome, l'estrazione AI e la tappa successiva): indizi per tipo, e due regole di prudenza imparate sul campo — i separatori `_` `.` `-` diventano spazi PRIMA di cercare (senza, «bolletta_enel_2026.pdf» non veniva riconosciuta: `_` e carattere di parola e i confini `\b` non esistono), e si assegna solo con margine di almeno due indizi sul secondo classificato (con margine di uno «bolletta_luce_e_gas.pdf» finiva nel gas, perdendo lo Scope 2). In caso di dubbio si CHIEDE.
- SMISTAMENTO: ogni tipo dichiara quali documenti alimenta e in quale sezione; l'hub dice sempre dove e finito («alimenta Carbon Footprint → Scope 2»). Se un documento non serve ai percorsi attivi lo si dice con garbo e resta in archivio.
- EFFETTO SUI FASCICOLI: le voci «per completare» passano ad ricevuto, le sezioni della bozza allo stato RICEVUTA (peso 0,75: il documento e in nostre mani ma il dato non e ancora dentro) e l'anello sale — verificato, Carbon da 33% a 46% e VSME da 30% a 58%. L'anello ha ora quattro livelli. Il caricamento vive anche dentro il fascicolo.
