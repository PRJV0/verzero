...
- PRIMO IMPATTO IN HOME — L'ONDA E UN FASCIO CHE MUTA (iterazione 3, 21/08/2026). La forma NON e piu una somma di sinusoidi ma viene da un campo di RUMORE coerente (Perlin 3D scritto in casa, `src/lib/rumore.ts`, nessuna libreria) campionato con coordinate che scorrono nel tempo. Motivo: una somma di sinusoidi, per quante ne metti, resta riconoscibile in pochi secondi — l'occhio ci trova il ritmo.
- COSA MUTA, con scale temporali tutte diverse fra loro cosi non c'e un battito comune: la linea si deforma, l'ampiezza cresce e si smorza, la larghezza si dilata e si restringe (escursione misurata 56px di fascio), la densita si concentra in punti che si spostano, e ogni tanto il fascio si sdoppia in due filamenti che si riuniscono. Le particelle NON sono disegnate sulla forma: la inseguono con INERZIA e smorzamento, quindi arrivano in ritardo — ed e il ritardo a rendere il movimento morbido. Strappo misurato: 0,15-0,20px per fotogramma, cioe nessuno scatto.
- PRESENZA: raggi da 0,8 a 5,9px con distribuzione non uniforme (molte piccole, poche grandi), opacita di base piu alta, e il 6-8% di particelle IN PRIMO PIANO piu grandi e piu opache con un alone che le fa vibrare sul bianco. Le dimensioni variano anche nel tempo e nello spazio: dove il fascio si addensa le particelle sono piu piccole e fitte, dove si rarefa piu grandi e distanziate.
- DUE CALIBRAZIONI, confrontabili dal vivo con `?onda=contenuta` sulla home: DECISA (predefinita) e CONTENUTA (stesso movimento, opacita al 62%, raggi al 78%). Il difetto segnalato era che l'onda non si vedeva, e un difetto si corregge per intero.
- LA MASCHERA DEL TESTO, tarata per RICERCA e non a occhio. Misurando la velatura al variare dei parametri si e scoperto che il `minimo` — quanto resta dell'onda SOTTO le lettere — non cambia il risultato di un punto da 0,012 a 0,08: a sporcare il testo sono le particelle che stanno NEGLI SPAZI fra le righe e ci arrivano col bagliore, largo tre raggi. Conta l'ESTENSIONE, e soprattutto quella verticale (ry 1,55 → 1,9: contrasto sul telefono da 4,16:1 a 5,09:1). Siccome il `minimo` e gratis si tiene alto (0,06): dietro le lettere l'onda resta un velo visibile, che e il punto.
- REGOLA PER SCHERMI STRETTI: sotto i 640px una maschera SOLA sul blocco, non una per riga. Su un telefono il claim occupa quasi tutta la larghezza, quindi fra una riga e l'altra non c'e spazio in cui l'onda si possa vedere — e le particelle che stanno li arrivano sul testo col bagliore e basta. Sul desktop invece l'onda passa fra le righe, ed e quello che fa galleggiare il claim.
- CONTRASTO MISURATO IN MOVIMENTO, nel punto peggiore, su tre minuti di simulazione con l'inerzia vera e quattro larghezze (375, 1280, 1440, 2560): 6,03-6,15:1 sul testo tenue e 8,3-8,5:1 su «cloud». Verificato anche sui PIXEL VERI del canvas nel browser: fondo peggiore rgb(241,245,244), 6,09:1 e 8,37:1.
- COSTO PER FOTOGRAMMA: 0,10-0,32ms di calcolo piu circa 0,3ms di rasterizzazione a dpr 2, cioe il 4% dei 16,7ms disponibili. 320 particelle sotto i 640px, 520 fino a 1100, 700 fino a 1800, 780 oltre. La forma del fascio si calcola UNA volta per particella per fotogramma e si ricorda: senza memoria veniva valutata quattro volte e il calcolo costava 0,77ms invece di 0,32.
