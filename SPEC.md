...
- PRIMO IMPATTO IN HOME — ONDA DI PARTICELLE (rifacimento del 20/08/2026, sostituisce la direzione C+B precedente). Claim TESTUALE su fondo bianco: «I tuoi consulenti in cloud.» in display grande, sottotitolo «La crescita della tua azienda, in abbonamento. Zero effort: bastano i documenti che hai gia.» con la sola parola «effort» in corsivo menta. La sostituzione grafica di «zero» con lo zero del marchio E STATA RIMOSSA: costringeva a decifrare una parola invece di leggerla, e in un claim la leggibilita viene prima di qualunque idea.
- L'ONDA: canvas a piena larghezza, 220 particelle su telefono / 380 su tablet / 560 da 1100px in su, distribuite lungo una curva di due sinusoidi di frequenza non multipla. Ogni particella ha fase, frequenza, ampiezza, velocita e profondita proprie; le due oscillazioni verticali stanno in rapporto aureo, quindi il movimento non si ripete in modo percepibile. Densita e ampiezza massime al centro e smorzate ai lati; menta solo per una minoranza e solo nella parte centrale, con transizione continua. Nessuna libreria: codice nostro, `src/lib/onda.ts` per la geometria e `src/components/onda-particelle.tsx` per il canvas.
- RESA: buffer alla risoluzione vera dello schermo (devicePixelRatio fino a 2, mai scalature al ribasso) e particelle disegnate come sprite con gradiente radiale pre-renderizzato in otto gradazioni — una `drawImage` a particella invece di un `arc()` con gradiente creato ogni fotogramma. I bordi morbidi e i 60fps sono la stessa decisione, non due.
- LA FASCIA STA SOTTO IL TESTO, SEMPRE, e l'ancoraggio e al fondo in PIXEL: con una frazione dell'altezza la fascia saliva sul sottotitolo appena la sezione cresceva (caratteri che caricano, claim che va a capo). `RISERVA_FONDO` e calcolata dalle ampiezze e l'hero la usa come padding inferiore: se un giorno la fascia diventa piu alta, il padding cresce con lei. `scripts/test-onda.mjs` verifica il legame — 37 prove su tre larghezze (375, 1280, 2560).
- IL PRIMO FOTOGRAMMA SI DISEGNA SUBITO, non dal requestAnimationFrame: assegnare width/height azzera il buffer, e un contesto senza canale alfa si azzera a NERO PIENO — in una scheda aperta in secondo piano l'hero sarebbe rimasto un rettangolo nero fino al primo sguardo.
