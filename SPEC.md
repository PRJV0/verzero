...
**Tetti di spesa — invisibili al cliente.** Tre ambiti con soglia (si avvisa) e tetto (ci si ferma): **pratica** $5/$15, **organizzazione nel mese** $20/$60, **giorno sull'intero servizio** $50/$150; si guarda prima il giorno, perché fermare per il tetto più generale dice che il problema non è di quel cliente. Nessun messaggio al cliente nomina limiti, quote o costi: legge che la lettura è in coda e riprende a breve — che è vero — mentre l'allarme coi numeri va in `motore_allarmi`, e il cruscotto `/dashboard/motore` (barriera `is_admin()` sulla RLS, non nel codice della pagina) mostra costo per cliente, costo ed errori per tipo di documento e allarmi non visti. **Mai attribuire all'ambiente un limite che è di budget.**
