...
- DMARC pubblicato su `_dmarc.verzero.it` come `v=DMARC1; p=none; rua=mailto:ciulliluca@hotmail.it` (dig, 20/08/2026). L'indirizzo e quello personale del fondatore perche `dmarc@verzero.it` non esiste ancora.
- VINCOLO APERTO — I RAPPORTI NON ARRIVERANNO COSI. La casella `rua` sta su un dominio diverso da quello del record, e in quel caso la RFC 7489 §7.1 impone a chi genera i rapporti di verificare che il dominio di destinazione acconsenta, cercando un TXT `v=DMARC1` su `verzero.it._report._dmarc.hotmail.it`. Quel record non esiste (verificato) e lo potrebbe pubblicare solo Microsoft: i reporter che applicano la regola — Google fra questi — non spediranno nulla. Il DMARC resta valido come politica, ma la raccolta e ferma.
- USCITA: una casella o un semplice inoltro `dmarc@verzero.it` sul dominio (stesso dominio del record: nessuna autorizzazione richiesta), oppure un servizio di analisi dei rapporti, che pubblica da se l'autorizzazione sul proprio dominio. Da preferire comunque a una casella personale: i rapporti aggregati sono XML compressi, decine al giorno, illeggibili a mano.
- Il senso di questo primo gradino resta leggere chi spedisce a nome nostro prima di stringere a `p=quarantine`: finche i rapporti non arrivano, quel passaggio non si puo decidere sui dati.
