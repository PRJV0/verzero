...
**La mappa documentale completa** — venti tipi con famiglia, forma, cosa si estrae, percorsi serviti e attesa realistica di qualità — è **generata dal registro** in `docs/tassonomia-documentale.md` (`scripts/mappa-documentale.mjs`, con `--controlla` che fallisce se il file diverge dal codice). Cinque tipi si sanno leggere oggi: bolletta elettrica, visura, organigramma, dati di organico, registri di formazione. Gli altri sono **dichiarati**: archiviati, riconosciuti e smistati come sempre, ma non ancora letti — e il portale non lascia credere il contrario.
...
**Due forme.** *Scheda* (campi fissi: bolletta, visura) e *tabella* (N righe della stessa forma: formazione, organico, organigramma, manutenzione). La forma cambia lo schema, la banca dati (`document_fields.riga`) e soprattutto **come si conferma**. Nelle tabelle confidenza e provenienza sono **per riga**, non per cella: venti righe per sei colonne farebbero centoventi involucri e un'interfaccia di conferma inutilizzabile — e la riga è l'unità che l'occhio legge sul foglio.

**Acquisizione da fotocamera.** Accanto al caricamento file, in Documenti e dentro i fascicoli: sul telefono apre la fotocamera posteriore (`capture`), su desktop resta un normale selettore. Scatti multipli con miniature, riordino, rifacimento pagina per pagina; alla conferma **un solo documento** — PDF multipagina cucito nel browser, col JPEG incastonato senza ricomprimerlo. Prima dell'invio ogni pagina è misurata: ritaglio sul foglio, raddrizzamento, nitidezza, luce, pixel residui (soglia **1.240 px** = 150 dpi su A4). Nel dubbio non si ritaglia; gli avvisi invitano a rifare e **non bloccano**. Il documento è marcato `da_fotocamera` e nel fascicolo si vede, con quante pagine.

**Manoscritto — regola inviolabile.** Ogni dato letto da una scrittura a mano è marcato `fonteLettura: "manoscritto"`, riceve **confidenza massima 0,6** imposta dal nostro codice (non chiesta al modello), richiede **conferma esplicita** e non entra in nessun calcolo prima di quella. Nessun automatismo di conferma massiva può includerlo. Se la grafia non si legge, il campo resta vuoto con la ragione e il cliente lo compila vedendo l'immagine accanto. **Ma il manoscritto è un caso normale, non un degrado**: in una PMI è metà dell'archivio, e se confermare venti righe costasse venti viaggi fra schermo e foglio nessuno lo farebbe — una regola che nessuno rispetta non protegge nessuno. Perciò la conferma obbligatoria si difende rendendola VELOCE: documento affiancato e aperto sulla pagina della riga, una riga alla volta con accanto la riga com'è scritta, tastiera sufficiente (Invio conferma e avanza, E corregge, X scarta), interfaccia che avanza subito col salvataggio che la insegue, avanzamento visibile. Obiettivo dichiarato: venti righe in un minuto, non in venti. Il blocco «conferma quelle che tornano» esiste e **non tocca mai il manoscritto**.

**Il triage, prima dell'estrazione.** Ogni documento passa da un primo sguardo — modello leggero, quattro campi, nessuna estrazione — che decide se leggerlo. Tre esiti fermano tutto: **dati particolari** (art. 9 GDPR: salute, giudiziari, biometrici, documenti d'identità) dove **la pertinenza non conta** e non si legge mai; **non pertinente**, che si archivia senza leggere; **illeggibile**, fermato prima di spendere l'estrazione. L'ordine non è casuale: l'art. 9 si guarda per primo, perché un certificato medico illeggibile o pertinente resta un certificato medico. Del contenuto non resta nulla — nel database l'esito e la categoria, cioè metadati sulla decisione — e al cliente si dice in chiaro, con la rimozione a un clic accanto al messaggio. Costo misurato **$0,0032 a documento**: si ripaga sui documenti cari (tabelle e scansioni: basta fermarne il 5-6%), non su quelli a buon mercato — e va detto così, perché chiamarlo «risparmio» sarebbe vero a metà.
...
**Generazione.** L'elaborato esce con struttura conforme alla norma, designazione ed **edizione vigente prese da `src/lib/norme.ts`** (mai scritte a mano), metodologia dichiarata, tracciabilità di dati e fonti, pagina di validazione professionale. Prima della consegna gira un **controllo di conformità bloccante**: se manca una sezione obbligatoria, o una designazione citata risulta ritirata, o un valore non ha fonte e stato `confermato`, il documento non si genera e si elenca cosa manca. Uscita PDF impaginato sempre, DOCX dove il cliente deve poter intervenire (manuali e procedure). **Implementata** in `src/lib/elaborato/` (docs/motore.md §5): il modello dichiara per ogni sezione come si compone (`componi`), i compositori registrati leggono i soli dati confermati, e ogni valore porta una **sigla di provenienza** — D letto da un documento dell'impresa, B recuperato da una banca dati, C calcolato, I inserito dall'impresa — che rimanda al registro delle fonti in appendice. Il controllo sta in `consegna.ts`, separato da chi compone, e ogni mancanza dice chi rimedia, che cosa e dove; i renderer accettano solo un documento che l'ha superato. I fattori di emissione vengono da un registro verificato sulle fonti ufficiali (`src/lib/calc/fattori.ts`). Prove: `scripts/test-elaborato.mjs`; documento d'esempio: `scripts/collaudo-elaborato.mjs`.

**Marchio del cliente.** Modelli separati dai contenuti; impostazioni a livello di organizzazione riusate su tutti i suoi documenti; logo con controlli di formato, risoluzione e sfondo; colori sociali con **contrasto verificato** per la stampa; anteprima prima della generazione; e impostazione neutra predefinita che resta professionale per chi non carica nulla. **Implementato** (`veste.ts`, `logo.ts`, Impostazioni → Veste dei documenti): contrasto misurato su carta da ufficio per ruolo — titoli 4,5:1, filetti 3:1, fasce 1,3:1 — con ripiego al neutro per il solo ruolo che non regge, detto col numero; logo rifilato, SVG rasterizzato e rifiutato se porta script o risorse esterne, chiaro su trasparente rifiutato, a bassa risoluzione mostrato più piccolo. Il marchio Verzero resta nel colophon, nei suoi colori, mai mescolato a quello del cliente.

**Volumi.** Due corsie: interattiva per chi sta guardando, differita in coda (Batch API, metà costo) per i lotti e le riletture. Coda con idempotenza su `(documento, lavoro, versione schema)`, ritentativi con arretramento **solo sugli errori ritentabili**, parallelismo controllato per organizzazione, segmentazione oltre le 100 pagine. Log tecnico in tabella `extractions`, di solo back-office: documento, modello, token, esito, durata, costo calcolato dai token effettivi.

**Versatilità per ambiti futuri.** Il Motore è una macchina per leggere documenti e comporre elaborati conformi: non ha nulla di specificamente ambientale. Gli ambiti che verranno — Modello 231, privacy e GDPR, sicurezza informatica — entrano come CONFIGURAZIONE, in cinque elenchi dichiarativi (ambito, tipi di documento, campi coi loro vincoli, norme, modelli di elaborato) e senza toccare una riga di pipeline. La struttura degli elaborati è **dato** (`src/lib/elaborati.ts`), non più sei funzioni scritte a mano; i controlli di plausibilità sono **vincoli dichiarati** sui campi (`min`, `max`, `formato`, `nonSupera`, `dentroLAnno`) applicati da un verificatore che non conosce i domini; il controllo di conformità legge il modello e non sa che cosa sia un Carbon Footprint. La prova è eseguibile: `scripts/test-ambiti.mjs` aggiunge il Modello 231 usando solo dati e verifica che estrazione, bozza e conformità funzionino. Procedimento completo in `docs/motore.md` §12.

**Limiti di uso corretto — contrattuali, in documenti e mai in valuta.** Ogni percorso attivo comprende, per anno di rendicontazione, **150 documenti elaborati** e **20 generazioni**; le dotazioni dei percorsi si sommano. Sono sei e sette volte l'uso reale misurato (25 documenti, 3 generazioni), perché il loro scopo non è razionare ma dare un tetto certo al costo variabile. Il superamento è a gradini e non blocca mai: oltre la dotazione le letture entrano in **coda a bassa priorità** («arrivano un po' più tardi, ma arrivano tutte»); oltre il doppio si chiede di **sentirsi**, e il lavoro in corso continua. Contatore discreto in portale, in documenti. **Caso peggiore certo: $34,50 per percorso/anno**, lineare nei percorsi ($138 con quattro). Testo contrattuale in `/termini`, sezione «Quanto puoi elaborare» — **da far validare al legale**.

**Tetti di spesa — invisibili al cliente.** Tre ambiti con soglia (si avvisa) e tetto (ci si ferma): **pratica** $3/$60, **organizzazione nel mese** $30/$250, **giorno sull'intero servizio** $60/$400. Stanno SOPRA il caso peggiore contrattuale di proposito: un tetto tecnico più basso fermerebbe in silenzio un cliente che sta usando ciò che ha comprato. Chi consuma tanto lo ferma il contratto, con una telefonata; chi ha un difetto lo ferma il tetto, con un allarme; si guarda prima il giorno, perché fermare per il tetto più generale dice che il problema non è di quel cliente. Nessun messaggio al cliente nomina limiti, quote o costi: legge che la lettura è in coda e riprende a breve — che è vero — mentre l'allarme coi numeri va in `motore_allarmi`, e il cruscotto `/dashboard/motore` (barriera `is_admin()` sulla RLS, non nel codice della pagina) mostra costo per cliente, costo ed errori per tipo di documento e allarmi non visti. **Mai attribuire all'ambiente un limite che è di budget.**

**Riuso, che è un'altra cosa dai tetti.** Se nulla è cambiato dall'ultima versione — stessa impronta di dati confermati, documenti, edizioni delle norme e modello — non si rigenera: si riapre il documento esistente dicendo *«Nulla è cambiato dall'ultima versione: ti riapriamo il documento già generato. Ogni elaborazione ha un costo energetico e non ha senso spenderlo due volte per lo stesso risultato»*, **con accanto sempre** il modo di generare comunque. Se invece qualcosa è cambiato la rigenerazione avviene **senza obiezioni né messaggi**, dicendo cosa è cambiato; su cicli ripetuti ravvicinati un invito gentile che non blocca. Lo stesso principio è già attivo sulla rilettura di un documento.

**Tracciabilità e versioni.** Ogni elaborato è una versione con data, anno di rendicontazione, **edizioni delle norme copiate dentro** (non riferite al registro, che cambia), elenco delle fonti dei dati e stato di validazione. Rigenerabile quando cambiano dati o norme, dicendo **perché** una nuova versione è possibile. **Implementato** con `elaborati_versioni`, immutabile anche per il service role: una correzione o una validazione professionale sono revisioni nuove, e la validazione riusa il contenuto salvato invece di ricomporlo. Migrazione `20260915120000_elaborati_e_marchio.sql`, **da applicare al remoto**.
...
**12.A — SERVIZIO "AGGIORNA IL TUO DOCUMENTO ESISTENTE" (migrazione documentale — incide sulla progettazione del Motore).**
- Cosa e: l'impresa ha gia un manuale o un documento di sistema (fatto da altri, o da se) costruito su un'EDIZIONE SUPERATA della norma o non allineato a modifiche organizzative. Carica il documento esistente; il Motore lo legge, lo confronta con l'edizione in vigore, produce l'ANALISI DEGLI SCOSTAMENTI (cosa e cambiato, dove il documento non e piu allineato) e RIGENERA il documento aggiornato mantenendo l'impianto e i contenuti specifici dell'impresa; verifica professionale prima della consegna.
- PERCHE E STRATEGICO: apre il mercato di chi e GIA servito da altri (bacino enorme: tutte le imprese certificate con manuali datati) senza chiedere di ricominciare da zero; e il primo servizio in cui il valore si vede in pochi minuti; si aggancia al registro delle norme gia costruito (controllo-norme) che sa quali edizioni sono superate.
- IMPATTO SUL MOTORE (fase 2): la pipeline di estrazione deve gestire, oltre ai documenti-FONTE (bollette, visure), anche i documenti-OPERA: PDF/DOCX di manuali e procedure da cui estrarre STRUTTURA (indice, sezioni, riferimenti normativi citati, responsabilita, procedure) e non solo dati puntuali. Prevedere fin da subito due famiglie di schemi di estrazione: dati da documenti fonte, struttura da documenti opera.
- Prezzo: una tantum piu mantenimento (fascia micro indicativa 490 una tantum + canone di mantenimento allineato ai manuali), da validare. Vincolo: non si promette il superamento dell'audit; si dichiara che l'aggiornamento e all'edizione vigente della norma.

**12.B — MODALITA PRE-LANCIO DEL FUNNEL: RICHIESTA DI ATTIVAZIONE, NESSUN ADDEBITO.**
- Finche non ci sono societa costituita e pagamenti attivi, l'ultimo passo del funnel NON e un acquisto ma una RICHIESTA DI ATTIVAZIONE. Restano visibili e funzionanti: prezzi, selettore dimensione impresa, scelta canone mensile / unica soluzione annuale (servono a qualificare il lead e a fotografare la configurazione scelta).
- Schermata finale: ringraziamento per l'interesse + conferma che la richiesta e registrata + promessa di CONTATTO SUCCESSIVO da parte di Verzero per avviare il percorso + rassicurazione esplicita e in evidenza: "Nessun addebito: non ti verra addebitato nulla fino all'inizio effettivo delle attivita, che concorderemo insieme." Nessuna data promessa.
- L'ordine si registra come RICHIESTA (stato dedicato) con la configurazione completa (servizio, taglio, dimensione, formula, prezzo fotografato) — e diventa la lista dei potenziali piloti. Notifica email al fondatore a ogni richiesta.
- Email al richiedente: conferma della richiesta con il riepilogo della configurazione e la stessa rassicurazione sull'assenza di addebiti.
- Tutto il funnel completo di pagamento resta nel codice dietro interruttore di configurazione, per riaccenderlo senza riscritture.
