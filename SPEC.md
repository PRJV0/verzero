...
- STATO (18/08/2026, risolto): la causa del blocco era la CONFERMA EMAIL OBBLIGATORIA unita al limite di 2 email/ora del servizio integrato Supabase — oltre quella soglia falliva la signUp stessa, quindi l'account non nasceva proprio. Correzione applicata: `mailer_autoconfirm = true` (l'account del funnel e usabile subito con la sua password), password minima 8 con maiuscola/minuscola/numero imposta anche lato server, messaggi d'errore veritieri con rimedio, flusso reset completo, cambio password e uscita dalle Impostazioni. La conferma email resta REVOCABILE: si riattiva con un flag quando Resend e in funzione, se si vuole la verifica preventiva dell'indirizzo. I template email in italiano si applicano solo con SMTP proprio (limite del piano free): li imposta `scripts/configura-smtp.mjs`.
...
**12.J — TAPPA 2.1 ARRICCHIMENTO: ESITO DELLA VERIFICA DELLE FONTI (18/08/2026).**
Prima di scrivere codice ogni fonte e stata verificata sulla sua via legittima di accesso. Esito:
- VIES (Commissione Europea) — ATTIVA. API REST pubblica e ufficiale, senza chiave: valida la P.IVA e restituisce denominazione e sede legale registrate. E oggi l'UNICA fonte gratuita che dia dati anagrafici veri.
- ATECO/ISTAT — ATTIVA come decodificatore. La classificazione e dato pubblico e sta in casa; traduce un codice gia noto nel settore leggibile. Non scopre il codice: quello arriva dal Registro Imprese.
- Agenzia delle Entrate — SPENTA. L'accesso programmatico e in API Management, aperto a un numero ristretto di sperimentatori e poi ai soggetti accreditati SdI. Serve quell'accreditamento.
- INI-PEC — SPENTA. Il portale e libero per la consultazione umana; l'accesso applicativo o massivo passa da convenzione con InfoCamere. Serve stipularla.
- ACCREDIA — SPENTA, e qui non manca una chiave: le note legali VIETANO estrazione e reimpiego della banca dati, anche parziale e anche ripetuta su parti non sostanziali. La via corretta e un accordo; intanto i certificati li dichiara il cliente.
- Provider camerale (es. OpenAPI.it) — SPENTA, predisposta. Sblocca forma giuridica, ATECO, unita locali, dipendenti, capitale, bilanci. Serve un account con credito e OPENAPI_TOKEN. Si interroga SOLO dopo l'incasso dell'ordine.
Regola permanente: mai scraping contro i termini d'uso. Meglio un campo vuoto che un dato indifendibile — vendiamo esattamente la difendibilita dei dati.
