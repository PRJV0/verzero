...
- PRIMO IMPATTO IN HOME — ONDA DI PARTICELLE (rifacimento del 20/08/2026). Claim TESTUALE su fondo bianco, con l'onda che ATTRAVERSA tutto l'hero e passa DIETRO le lettere: la curva sta al 47% dell'altezza (42% sotto i 640px) e fra cresta e ventre la fascia occupa il 55-57% dell'hero. Il canvas sborda di 90px per lato, cosi l'onda entra ed esce dal campo invece di cominciare e finire dentro l'inquadratura.
- IL CLAIM E IL PROTAGONISTA: 40px sotto i 640px, 52 fino a 768, 72 fino a 1280, 80 oltre; interlinea 1,02, crenatura -0,022em, peso pieno del display. «cloud» in pino pieno col resto in pino scuro all'80% — variante (a). Scartata la (b) col corsivo menta perche sopra un fondo mosso comanda il contrasto: cosi la parola-chiave e la piu scura della riga (7,4-7,6:1 misurati contro 5,4-5,5:1 del resto), mentre l'accento menta sarebbe stato il punto piu chiaro proprio sull'unica parola che deve reggere da lontano — e la menta e gia la firma di «effort» e dell'onda. Ingresso scaglionato PER RIGA, mai per lettera.
- IL TESTO NON VIENE EVITATO, VIENE PROTETTO: una maschera PER RIGA (`data-onda-maschera`), non una sul blocco intero — con una sola maschera si spegnevano anche gli spazi fra le righe e dietro il claim non restava niente da vedere. La maschera usa una norma di ordine 6, non un'ellisse, perche deve seguire la forma di un rettangolo: con l'ellisse gli ANGOLI delle righe restavano scoperti, ed e li che la misura del contrasto trovava il fondo peggiore. Piu un alone bianco radiale e un'ombra bianca sfumata sul testo: nessun riquadro visibile.
- CONTRASTO MISURATO IN MOVIMENTO, nel punto peggiore, non su fondo uniforme: 80 istanti campionati per quattro larghezze (375, 1280, 1440, 2560) danno 4,98-5,41:1 sul testo tenue e 6,85-7,45:1 su «cloud». La soglia della velatura non e un numero scelto a mano: si ricava risolvendo il contrasto minimo AA (22,5%), e la velatura massima misurata e 12,5-17,6%. La stessa misura letta dai pixel veri del canvas nel browser da 5,52:1 e 7,60:1 — il modello e tarato sulla realta.
- DINAMICA: tre sinusoidi con periodi non multipli sulla portante e tre per ogni particella (rapporti aurei), fase e ampiezza proprie, e una minoranza (12%) che corre quasi al doppio. Densita e ampiezza massime al centro, smorzate ai lati. Spostamento massimo fra due fotogrammi a 60fps: 0,25px — nessuno scatto, nessun rimbalzo.
- COSTO PER FOTOGRAMMA MISURATO: 0,03-0,06ms di calcolo (Node) piu 0,2-0,3ms di rasterizzazione a dpr 2 (browser, mediana su 90 fotogrammi) — circa il 2% dei 16,7ms disponibili, alle quattro larghezze. 260 particelle sotto i 640px, 420 fino a 1100, 560 fino a 1800, 620 oltre.
- FIRMA VISIVA RIUSABILE: una sola implementazione (`src/lib/onda.ts` + `src/components/onda-particelle.tsx`) e una sola configurazione centrale (`PRESET`), mai una copia. Riusata nella sezione del Motore (palette invertita, carattere tecnico, 30%), nella fascia del Sigillo (22%, scura), in Chi siamo (24%), in coda alla home sul modulo lista d'attesa (20%) e nelle intestazioni delle pagine servizio (20%). REGOLA: dove c'e testo denso o dati, l'onda sta sotto il 25% o non c'e; sulle sezioni scure si inverte la palette (bianco con accenti menta accesa).
- IL PRIMO FOTOGRAMMA SI DISEGNA SUBITO, fuori dal requestAnimationFrame: in una scheda aperta in secondo piano il rAF non parte finche non la si guarda. Con «riduci movimento» si disegna quel fotogramma e basta, in una posizione armoniosa (t=7,5s).
