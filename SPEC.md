...
- DMARC pubblicato su `_dmarc.verzero.it` come `v=DMARC1; p=none`. DA COMPLETARE: manca `rua=mailto:dmarc@verzero.it`. Senza indirizzo di rapporto il record e inerte — non applica politiche (e p=none) e non raccoglie niente, mentre il senso di questo primo gradino e proprio leggere chi spedisce a nome nostro prima di stringere a `p=quarantine`.
